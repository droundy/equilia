use super::{Chunk, IsRawColumn, ReadEncoded, Storage, StorageError, WriteEncoded, U64_16_MAGIC};

#[derive(Clone)]
pub(crate) struct U64_16Column {
    storage: Storage,
    current_row: u64,
    n_rows: u64,
    n_chunks: u64,
...
impl From<&[u64]> for U64_16Column {
    fn from(vals: &[u64]) -> Self {
        let mut bytes = Vec::<u8>::new();
        U64_16Column::encode(&mut bytes, &super::run_length_encode(vals)).unwrap();
        println!("encoded is {bytes:?}");
        let storage = Storage::from(bytes);
        U64_16Column::open(storage).unwrap()
    }
}
impl Iterator for U64_16Column {
...
impl U64_16Column {
    fn transposed_next(&mut self) -> Result<Option<Chunk<u64>>, StorageError> {
        ...
        let value = self.v_min + self.storage.read_u16()? as u64;
...
impl IsRawColumn for U64_16Column {
    type Element = u64;
...
impl TryFrom<Storage> for U64_16Column {
...
fn encode_u64() {
    ...
    let bc = U64_16Column::from(&bools[..]);
    ...
    let chunks: Vec<(u64, u64)> = bc
        ...
        .collect();
    <U64_16Column as IsRawColumn>::encode(&mut encoded, chunks.as_slice()).unwrap();

    let storage = Storage::from(encoded.clone());
    let bc2 = U64_16Column::open(storage.clone()).unwrap();
    assert_eq!(
    ...
    <U64_16Column as IsRawColumn>::encode(&mut f, chunks.as_slice()).unwrap();
